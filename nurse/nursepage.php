<?php session_start();
$assigned_to_sess = $_SESSION["assigned_dr"];
$_SESSION["assigned_dr"] = $assigned_to_sess;
...
<body>
    ...
    <div  id="profile-info">
        <div class="curtain">
            ...
            <div class="profile">
                <img src="/MedCon/img/nurse.png" alt="Image" class="pfp-pic">
                <p>Welcome back, Nurse <span><?php echo $name_member; ?></span></p>
                <p id="hosp"><?php echo $hospname?></p>
            </div>
        </div>
                    <?php 
                include '../admin/conn.php';
                $sql = sprintf("SELECT * FROM nurse WHERE nurse_id='%s'",
                $mysqli->real_escape_string($nurse_id));
                $result =$mysqli->query($sql);
                $drres = $result->fetch_assoc();
                $dremail = $drres["email"];
                $drphone = $drres["phone"];
                $assignedto = $drres["assigned_dr"];
                $sqldr = sprintf("SELECT * FROM doctor WHERE dr_id='%s'", $mysqli->real_escape_string($assignedto));
                $resultdr = $mysqli->query($sqldr);
                $drresdr = $resultdr->fetch_assoc();
                $assigneddr = $drresdr["fname"] . " " . $drresdr["lname"];
                $drfname = $drres["fname"];
                $drlname = $drres["lname"];
                $_SESSION["nurse_id"] = $drres["nurse_id"];
                $nurseIDTOSEND = $drres["nurse_id"];
                    ?>
      ...
      <div class="modal-body" style="background-color: #0a2737;">
        <form class=" g-3 " enctype="multipart/form-data"  id="editModalNurse">
            <div class="row col-8">
                ...
                <input type="text" hidden value="<?php echo $nurseIDTOSEND; ?>" id="nurseid" >
          ...
          <div class="row col-8">
            ...
            <input type="text" class="form-control readonly" id="inputAddress2Modal" readonly value="<?php echo $hospname; ?>">
        </div>
        <hr>
        <div class="row col-md-8" id="lastcol">
            <label for="inputCityModal" class="form-label">Assigned Dr.</label>
            <input type="text" class="form-control readonly" id="inputCityModal" readonly value="<?php echo $assigneddr; ?>">
          </div>
...
<div class="content home" id="modalhome">
    ...
    <h2 style="color: #fff; text-align: center; width: 100%;">Schedule an Appointment:</h2>
    <form id="appointment-form" action="add_appt.php" method="POST">
        <!-- Left Column -->
        ...
        <div class="col-md-6 d-flex flex-column right-column flex-grow-1">
            <div class="form-group" id="choose">
                ...
                <select class="form-select" id="patient" name="patient" required>
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<option value='{$row['patient_id']}'>{$row['fname']} {$row['lname']}</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="form-group mt-auto">
                <button type="submit" class="btn btn-primary w-100" id="apptBtn">Set Appointment</button>
            </div>
        </div>
    </form>
</div>
...
<div class="tests-page-container">
   ...
   <script src="edit.js"></script>
    ...
    <script src="nurse_noti.js"></script>
