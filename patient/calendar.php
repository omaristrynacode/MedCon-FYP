...
class Calendar {
    ...
    public function request_event($drid, $nurse_id, $patient, $date, $time, $reason, $hospital) {
        $stmt = $this->db->prepare("INSERT INTO appointmentrequest (dr_id, nurse_id, patient_id, date, time, reason, hosp_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
 ...
 $assigned_to = $_SESSION["patient_id"];

        $sql = sprintf("SELECT * FROM appointment WHERE patient_id='%s'",
